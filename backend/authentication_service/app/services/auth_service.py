...
class AuthService:
    ...
    async def register_user(self, user: UserCreate) -> Dict:
        result = await self.db.execute(select(User).where(User.email == user.email or User.username == user.username))
    ...
    async def login_user(self, email: str, password: str) -> Token:
        ...
        if not user.email_verified:
            raise HTTPException(status_code=status.HTTP_423_LOCKED, detail="Email not verified")
