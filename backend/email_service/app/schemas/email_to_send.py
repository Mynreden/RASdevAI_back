...
class EmailToSend(BaseModel):
    ...
    cc: Optional[list[EmailStr]] = Field(description="Копия письма (список email)")
    bcc: Optional[list[EmailStr]] = Field(description="Скрытая копия письма (список email)")
    attachments: Optional[list[str]] = Field(description="Список путей к вложениям или base64 (по желанию)")
